...
WAVEFIELD_SAMPLE_INTERVAL = 1
def forward_model_mpi(epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps=1000, save_wavefield=False, return_illumination=False):
    """
    MPI并行版本的2D FDTD正演模拟
    使用MPI并行处理多个shot_idx
    """
    if not MPI_AVAILABLE:
        return forward_model(epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps, save_wavefield, return_illumination)
    ...
    local_illumination = np.zeros((xl, zl)) if return_illumination else None
        ...
        for local_shot_idx, source_pos in enumerate(local_source_list):
            ...
            for step_idx, (ey_field, receiver_data) in enumerate(time_loop_gen):
                ...
                if return_illumination:
                    ey_np = np.array(ey_field)
                    ey_valid = ey_np[npml:npml+xl, npml:npml+zl]
                    local_illumination += ey_valid**2
                if save_wavefield:
    ...
    if save_wavefield and return_illumination:
        return local_data, local_wavefield_data, illumination
    if save_wavefield:
        return local_data, local_wavefield_data
    if return_illumination:
        return local_data, illumination
    return local_data

def forward_model(epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps=1000, save_wavefield=False, return_illumination=False):
    """
    ...
    if MPI_AVAILABLE and MPI.COMM_WORLD.Get_size() > 1:
        return forward_model_mpi(epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps, save_wavefield, return_illumination)
    ...
    illumination = np.zeros((xl, zl)) if return_illumination else None
    if save_wavefield:
        wavefield_data = []
    ...
    if save_wavefield and return_illumination:
        return data, np.array(wavefield_data), illumination
    if save_wavefield:
