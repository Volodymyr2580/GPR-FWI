...
def parse_args(argv=None):
    ...
    parser.add_argument("--alpha-eps", type=float, default=0.0)
    ...
    parser.add_argument("--num-epochs-stage1", type=int, default=3001)
...
def main(argv=None):
    ...
    freq = 4e8
    # 移除未使用的浅层固定参数
    l_size = args.fixed_top_rows  # 浅层固定深度
    ...
    sigma_true = np.ones((xl+l_size, zl)) * 0
    ...
    if rank == 0:
        ...
        plt.imshow(epsilon_true.detach().cpu().numpy(), aspect='auto', cmap='jet', vmin=1, vmax=max_eps)
    ...
    if rank == 0:
        plt.figure(figsize=(6, 4))
        plt.imshow(eps_input.detach().cpu().numpy(), aspect='auto', cmap='jet', vmin=min_eps, vmax=max_eps)
        plt.title('Input Epsilon Model')
        plt.colorbar()
        plt.tight_layout()
        plt.savefig(f'{path}epsilon_input.png')
        plt.close()
    # ========== 生成观测数据（MPI并行化） ==========
    if rank == 0:
        print("生成观测数据...")
    ...
    # 计算padding
    target_height, target_width = xl + l_size, zl
