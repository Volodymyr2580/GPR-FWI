...
def parse_args(argv=None):
    ...
    parser.add_argument("--fixed-top-rows", type=int, default=L_SIZE)
...
def main(argv=None):
    ...
    if rank == 0:
        ...
        plt.imshow(epsilon_true.detach().cpu().numpy(), aspect='auto', cmap='jet', vmin=1, vmax=6)
            ...
            if rank == 0:
                ...
                if epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                    ...
                    if outputs_eps_use.grad is not None:
                        ...
                        # 梯度图（归一化后）
                        plt.figure()
                        plt.imshow(grad_np, aspect='auto', cmap='RdBu_r', 
                                  vmin=-1, vmax=1)
