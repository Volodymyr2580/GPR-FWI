...
from gpr_inversion.io import ensure_unique_dir
...
class ForwardModelFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps, alpha_eps, alpha_sig):
        """
        前向传播：执行FDTD正演模拟
        """
        ...
        ctx.illumination = illumination  # 保存照明项
        ctx.H = H  # 保存展平的H向量
    ...
    def backward(ctx, grad_output, grad_H):
        ...
        illumination = ctx.illumination  # 获取保存的照明项
        H = ctx.H  # 获取保存的展平H向量
        ...
        # forward函数有10个参数：epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps, alpha_eps, alpha_sig
        grad_eps = grad_eps_tensor / torch.max(torch.abs(grad_eps_tensor)) * alpha_eps
        if grad_sig_tensor is not None:
            grad_sig = grad_sig_tensor / torch.max(torch.abs(grad_sig_tensor)) * alpha_sig
        return grad_eps, grad_sig, None, None, None, None, None, None, None, None, None, None
...
class TikhonovRegularizationFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        alpha_tikhonov_eps = args.alpha_tikhonov_eps  # Tikhonov正则化参数
...
# 兼容旧函数名：现在不删除旧结果，而是返回一个唯一目录。
def ensure_clean_dir(target_dir):
    return str(ensure_unique_dir(target_dir))
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run a migrated Marmousi UNet epsilon-only experiment.")
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--data-path", type=str, default="modelv1.bin")
    parser.add_argument("--output-dir", type=str, default="result_gpr_mpi")
    parser.add_argument("--learning-rate-eps", type=float, default=1e-4)
    parser.add_argument("--alpha-eps", type=float, default=0.1)
    parser.add_argument("--alpha-sig", type=float, default=0.0)
    ...
    parser.add_argument("--num-epochs-stage1", type=int, default=6001)
...
def main(argv=None):
    ...
    # 创建结果文件夹（仅在rank 0执行）。存在旧结果时自动使用 _run001 后缀。
    if rank == 0:
        base_output_dir = str(ensure_unique_dir(args.output_dir))
    ...
    # ========== 训练设置 ==========
    learning_rate = args.learning_rate_eps  # 学习率
    alpha_eps = args.alpha_eps  # epsilon 模型更新步长
    ...
    if rank == 0:
        ...
        result_dir = str(ensure_unique_dir(result_dir))
    ...
    #真实模型参数
    marmousi_model = np.fromfile(args.data_path, dtype=np.float32)
    ...
    epsilon_top_rows = np.tile(epsilon_top_row, (l_size, 1))
    ...
    sigma_mig = torch.zeros_like(sigma_true, requires_grad=False)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            if rank == 0 and net_1 is not None:
                ...
                net1_output_model = net1_output_model[:orig_height, :orig_width]
            ...
            outputs_eps_use.register_hook(make_grad_hook(l_size))
            ...
            d_syn, H = ForwardModelFunction.apply(
                ...
                dt, dx, dz, npml, freq, steps, alpha_eps, alpha_sig
            )
            if rank == 0:
            ...
            if rank == 0:
                ...
                if epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                    ...
                    plt.imshow(outputs_eps_use.detach().cpu().numpy(), aspect='auto', 
                              cmap='jet', vmin=min_eps, vmax=max_eps)
