...
# Serial JAX OverThrust top-50 fixed sweep on physical CUDA GPU 3.
# CUDA_VISIBLE_DEVICES=3 makes physical cuda:3 appear as logical cuda:0
# to both PyTorch and JAX inside this process.
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-3}"
export XLA_PYTHON_CLIENT_PREALLOCATE="${XLA_PYTHON_CLIENT_PREALLOCATE:-false}"
export JAX_FDTD_SHOT_BATCH_SIZE="${JAX_FDTD_SHOT_BATCH_SIZE:-1}"

DATA_PATH="${DATA_PATH:-data/overthrust/OverThrust.npy}"
ROOT_OUTPUT_DIR="${ROOT_OUTPUT_DIR:-outputs/overthrust/jax_top50_cuda3_sweep}"
...
COMMON_ARGS=(
  ...
  --device cuda:0
  --jax-device cuda:0
  --jax-shot-batch-size "${JAX_FDTD_SHOT_BATCH_SIZE}"
...
run_case() {
  ...
  python -m gpr_inversion.experiments.overthrust.mode2_eps_then_sig.twopara_epsfirst "${COMMON_ARGS[@]}" "$@" --output-dir "${output_dir}" 2>&1 | tee "${log_file}"
...
echo "All CUDA-3 serial sweep cases finished."
