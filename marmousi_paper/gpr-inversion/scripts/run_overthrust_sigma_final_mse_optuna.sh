...
# Search sigma hyperparameters using final-epoch sigma MSE only.
# Fixed epsilon-side settings: lrE1=1e-4, lrE2=2e-6, tvEps=0.04, l1D=0.2.
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/overthrust/optuna_jax_sigma_final_mse}"
STUDY_NAME="${STUDY_NAME:-overthrust_jax_sigma_final_mse}"
...
args=(
  ...
  --objective-source final
  --model-error-metric mse
  --eps-weight 0.0
  --sig-weight 1.0
  --mse-weight 1.0
  --ssim-weight 0.0
  ...
  --learning-rate-sig-min "${LR_SIG_MIN:-5e-7}"
  --learning-rate-sig-max "${LR_SIG_MAX:-1e-4}"
  --alpha-tv-eps "${ALPHA_TV_EPS:-0.04}"
  --alpha-tv-sig-min "${ALPHA_TV_SIG_MIN:-1e-3}"
