...
# ========== GPU加速支持 ==========
# 尝试导入GPU加速版本的forward，如果不可用则回退到CPU版本
try:
    import cupy as cp
    from utils.forward_gpu import forward_model_gpu, load_filtered_wavelet
    GPU_AVAILABLE = True
    
    # 全局变量：每个MPI进程使用的GPU ID
    _process_gpu_id = None
    
    # 包装函数，将根据进程的GPU ID调用
    def forward_model(*args, **kwargs):
        global _process_gpu_id
        gpu_id = kwargs.pop('device_id', _process_gpu_id if _process_gpu_id is not None else 0)
        return forward_model_gpu(*args, **kwargs, device_id=gpu_id, verbose=False)
    
    if rank == 0:
        print("=" * 70)
        print("✓ GPU加速已启用 - 使用CuPy加速FDTD计算")
        print("=" * 70)
    ...
    if rank == 0:
        print("=" * 70)
        print(f"⚠ GPU加速不可用，使用CPU模式: {e}")
        print("  请安装CuPy: pip install cupy-cuda11x 或 cupy-cuda12x")
        print("=" * 70)


from utils.gradient_gpu import compute_gradient_gpu, compute_tikhonov_gradient_gpu, compute_laplacian_2d_gpu
...
class ForwardModelFunction(Function):
    ...
    def forward(ctx, epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps):
        ...
        优化：如果输入已在GPU上，避免不必要的CPU-GPU传输
        """
        # 智能转换：如果已在GPU上，forward_model_gpu会零拷贝转换
        # 如果在CPU上，则转numpy
        if epsilon.is_cuda and GPU_AVAILABLE:
            # GPU张量，直接传递（forward_model_gpu会处理）
        ...
        # 执行正演模拟
        data, wave_field = forward_model(epsilon_input, sigma_input, source_list, receiver_list, 
                           dt, dx, dz, npml, freq, steps, save_wavefield=True, wavelet=None)
        
        # 调试：检查wave_field格式
        if wave_field is not None:
            wf_type = type(wave_field)
            wf_len = len(wave_field) if isinstance(wave_field, list) else 'N/A'
            if isinstance(wave_field, list) and len(wave_field) > 0:
                wf0_type = type(wave_field[0])
                wf0_len = len(wave_field[0]) if isinstance(wave_field[0], list) else wave_field[0].shape if hasattr(wave_field[0], 'shape') else 'N/A'
                print(f"[调试forward] wave_field类型={wf_type}, len={wf_len}, wave_field[0]类型={wf0_type}, len={wf0_len}")
        else:
            print(f"[警告forward] wave_field为None！")
    ...
    def backward(ctx, grad_output):
        ...
        # 计算本地梯度（使用CPU版本，稳定）
        grad_eps, _ = compute_gradient_gpu(
            epsilon_np,
            sigma_np,
            grad_output_np,
            source_list,
        ...
        # 调试：检查compute_gradient_gpu GPU版本返回的梯度
        grad_sum_computed = np.sum(grad_eps)
        grad_max_computed = np.max(np.abs(grad_eps))
        if grad_max_computed == 0:
            print(f"[警告] compute_gradient_gpu GPU版本返回的梯度全为0！grad_sum={grad_sum_computed:.6e}")
        ...
        # 调试：检查返回的梯度tensor
        grad_sum_return = grad_eps_tensor.sum().item()
        grad_max_return = grad_eps_tensor.abs().max().item()
        if grad_max_return == 0:
            print(f"[警告] ForwardModelFunction.backward返回的梯度全为0！grad_sum={grad_sum_return:.6e}")
...
class TikhonovRegularizationFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        tikhonov_grad_eps = compute_tikhonov_gradient_gpu(epsilon_np, dx, dz, alpha_tikhonov_eps)
        tikhonov_grad_sig = compute_tikhonov_gradient_gpu(sigma_np, dx, dz, alpha_tikhonov_sig)
...
def main():
    ...
    if torch.cuda.is_available() and GPU_AVAILABLE:
        ...
        if rank == 0:
            print("=" * 70)
            print("GPU分配策略: 仅使用GPU 0（单卡模式）")
            print("=" * 70)
            print(f"检测到 {n_gpus} 个GPU，{size} 个MPI进程")
            # 打印进程-GPU映射关系
            for r in range(size):
                gpu_id = 0
                props = torch.cuda.get_device_properties(gpu_id)
                print(f"  Rank {r} → GPU {gpu_id} ({props.name}, {props.total_memory/1024**3:.1f}GB)")
            print("=" * 70)
        ...
        # 设置FDTD正演计算使用的GPU ID
        global _process_gpu_id
        _process_gpu_id = process_gpu_id

        print(f"[Rank {rank}] 使用GPU {process_gpu_id}")
        
    elif torch.cuda.is_available() and not GPU_AVAILABLE:
        # PyTorch CUDA可用但CuPy不可用，只有rank 0使用GPU做UNet
        if rank == 0:
            device = torch.device('cuda:0')
            torch.cuda.set_device(0)
            print(f"[Rank {rank}] PyTorch使用GPU 0，但FDTD将使用CPU（CuPy不可用）")
        else:
            device = torch.device('cpu')
            print(f"[Rank {rank}] 使用CPU")
    else:
        device = torch.device('cpu')
        if rank == 0:
            print("CUDA不可用，所有进程使用CPU")
        print(f"[Rank {rank}] 使用CPU")
    
    if rank == 0:
        print(f"\n当前配置:")
        print(f"  MPI进程数: {size}")
        print(f"  GPU加速: {'✓ 已启用' if GPU_AVAILABLE else '✗ 未启用'}")
        if GPU_AVAILABLE and torch.cuda.is_available():
            print(f"  每个进程的GPU: 自动分配 (进程{rank} = GPU{process_gpu_id})")
        print(f"  提示: 使用 'watch -n 1 nvidia-smi' 监控GPU使用情况")
        print("=" * 70)
    ...
    #调整一下真实模型 - 将第100行的值复制到前100行（平滑浅层）
    initial_model_pad[:100,:] = initial_model_pad[100:101,:]
    ...
    for epoch in range(num_epochs):
        ...
        # ✓ 优化：梯度累积器在GPU上（避免GPU→CPU→GPU）
        accumulated_grad_eps = torch.zeros((410, 5820), dtype=torch.float32, device=device)
        ...
        for shot_idx in local_selected_shots:
            ...
            # ✓ 优化：直接在GPU上提取窗口（需要创建独立tensor以接收梯度）
            # 计算窗口位置
            ...
            # 调试：正演合成数据范围（每个epoch前几个炮点打印一次）
            if (rank == 0) and ((shot_idx - local_selected_shots[0]) < 3):
                dmin = d_syn_shot.min().item()
                dmax = d_syn_shot.max().item()
                drng = float(max(abs(dmin), abs(dmax)))
                print(f"[调试 epoch {epoch}] Shot {shot_idx} d_syn范围: [{dmin:.3e}, {dmax:.3e}], |max|={drng:.3e}")
            ...
            if epsilon_window.grad is not None:
                # 检查梯度是否全为0（调试）
                grad_sum = epsilon_window.grad.sum().item()
                grad_max = epsilon_window.grad.abs().max().item()
                
                if rank == 0 and shot_idx == local_selected_shots[0]:
                    print(f"[调试] Shot {shot_idx}: grad_sum={grad_sum:.6e}, grad_max={grad_max:.6e}")
                ...
                # 梯度为None，可能是计算图问题
                if rank == 0 and shot_idx == local_selected_shots[0]:
                    print(f"[警告] Shot {shot_idx}: epsilon_window.grad为None！检查ForwardModelFunction.backward")
        ...
        # 调试：检查累积后的梯度（在MPI同步前）
        if rank == 0:
            grad_sum_before_sync = accumulated_grad_eps.sum().item()
            grad_max_before_sync = accumulated_grad_eps.abs().max().item()
            print(f"[调试] Epoch {epoch}: MPI同步前 - grad_sum={grad_sum_before_sync:.6e}, grad_max={grad_max_before_sync:.6e}")
        
        # ✓ 优化：MPI同步梯度（在GPU上操作）
        if MPI_AVAILABLE and size > 1:
            # 梯度已经是torch tensor，需要转numpy同步
            grad_np = accumulated_grad_eps.detach().cpu().numpy()
            ...
            # 调试：检查MPI同步后的梯度
            if rank == 0:
                grad_sum_after_sync = accumulated_grad_eps.sum().item()
                grad_max_after_sync = accumulated_grad_eps.abs().max().item()
                print(f"[调试] Epoch {epoch}: MPI同步后 - grad_sum={grad_sum_after_sync:.6e}, grad_max={grad_max_after_sync:.6e}")
        ...
        # 确保padding区域的梯度为零（直接在GPU tensor上操作）
        ...
        # 调试：检查最终梯度
        if rank == 0:
            grad_sum_final = grad_eps_no_pad.sum().item()
            grad_max_final = grad_eps_no_pad.abs().max().item()
            print(f"[调试] Epoch {epoch}: 最终梯度 - grad_sum={grad_sum_final:.6e}, grad_max={grad_max_final:.6e}")
