...
def compute_gradient_mpi(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient=False, wavefield_data=None, global_start_idx=0):
    ...
    for local_shot_idx, (source_pos, receiver_pos) in enumerate(zip(local_source_list, local_receiver_list)):
        ...
        if wavefield_data is not None:
            ...
            # wavefield_data现在是嵌套列表，已经降采样过
            # wavefield_data 在MPI路径下是本进程局部的波场列表
            forward_wavefield = np.array(wavefield_data[local_shot_idx])
            
            # 伴随波场也进行降采样保存
            adjoint_list_full = [np.array(adj) for adj in reverse_loop_gen]
            # 降采样伴随波场
            adjoint_list = [adjoint_list_full[i] for i in range(0, len(adjoint_list_full), WAVEFIELD_SUBSAMPLE)][::-1]
            
            # 计算梯度（使用降采样后的波场）
            # 注意：forward_wavefield已经是降采样的，形状为 (n_time_subsampled, xl+2*npml, zl+2*npml)
            n_time_subsampled = len(forward_wavefield)
            
            # 计算时间导数（使用降采样后的时间步）
            dt_subsampled = dt * WAVEFIELD_SUBSAMPLE
            forward_diff = (forward_wavefield[2:] - forward_wavefield[:-2]) / (2 * dt_subsampled)
            
            for k in range(1, n_time_subsampled-1):
    ...
    for shot_idx, (source_pos, receiver_pos) in enumerate(zip(source_list, receiver_list)):
        ...
        if wavefield_data is not None:
            ...
            if shot_idx < len(wavefield_data) and len(wavefield_data[shot_idx]) > 0:
                ...
                # 伴随波场也进行降采样
                adjoint_list_full = [np.array(adj) for adj in reverse_loop_gen]
                # 降采样伴随波场（每WAVEFIELD_SUBSAMPLE步取一次）
                adjoint_list = [adjoint_list_full[i] for i in range(0, len(adjoint_list_full), WAVEFIELD_SUBSAMPLE)][::-1]
