...
def main(optimizer=None, opt_params=None, epochs=5001):
    ...
    class Muon:
        def __init__(self, lr=0.05, mu=0.95, ns=5, eps=1e-8):
    ...
    def run_once(name, params):
        ...
        for epoch in range(epochs):
            ...
            grad_eps, grad_sig = compute_gradient(model_eps, model_sig, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, False, wavefield_data)
        ...
        save_model_fig(model_eps, os.path.join(base, 'final_model_eps.png'), vmin=1, vmax=6, title=f'{name} Final')
        plt.figure()
        plt.plot(loss_list)
        plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.title(f'{name} Loss 曲线')
    ...
    if optimizer is None:
        defaults = {
            ...
            'Muon': {'lr': 0.05, 'mu': 0.95, 'ns': 5, 'eps': 1e-8}
        }
