...
def forward_model(epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps=1000, save_wavefield=False):
    """
    2D FDTD正演模拟，返回合成观测数据 (mode2: 常规排列)
    ...
    return: data (n_shots, n_receivers, n_steps), wavefield_data (如果save_wavefield=True)
    """
    xl, zl = epsilon.shape
