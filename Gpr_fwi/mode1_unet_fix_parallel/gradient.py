...
def compute_gradient_mpi(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient=False, wavefield_data=None):
    """
    MPI并行版本的梯度计算
    使用MPI并行处理多个shot_idx
    """
    ...
    local_residual = residual[start_idx:end_idx]
    ...
    for local_shot_idx, (source_pos, receiver_pos) in enumerate(zip(local_source_list, local_receiver_list)):
        ...
        # 伴随反传 - 严格按照原有方法
        reverse_loop_gen = reverse_time_loop(xl, zl, dx, dz, dt, sigma_extended.copy(), epsilon_extended.copy(), mu_extended.copy(), cpml_params, steps, receiver_pos_extended, local_residual[local_shot_idx, :])
        
        if wavefield_data is not None:
            # 使用全局shot索引访问波场数据
            global_shot_idx = start_idx + local_shot_idx
            
            # 安全检查波场数据
            if global_shot_idx < len(wavefield_data) and len(wavefield_data[global_shot_idx]) > 0:
                # wavefield_data现在是嵌套列表，不需要转换为numpy数组
                forward_wavefield = wavefield_data[global_shot_idx]
                ...
                if rank == 0:
                    print(f"Warning: Process {rank}: No valid wavefield data for shot {global_shot_idx}")
                    print(f"Warning: wavefield_data length: {len(wavefield_data)}, shot {global_shot_idx} data length: {len(wavefield_data[global_shot_idx]) if global_shot_idx < len(wavefield_data) else 'N/A'}")
...
def compute_gradient(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient=False, wavefield_data=None):
    ...
    if MPI_AVAILABLE and MPI.COMM_WORLD.Get_size() > 1:
        return compute_gradient_mpi(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient, wavefield_data)
    ...
    for shot_idx, (source_pos, receiver_pos) in enumerate(zip(source_list, receiver_list)):
        ...
        if wavefield_data is not None:
            ...
            if shot_idx < len(wavefield_data) and len(wavefield_data[shot_idx]) > 0:
                # wavefield_data现在是嵌套列表，不需要转换为numpy数组
                forward_wavefield = wavefield_data[shot_idx]
                forward_diff = (forward_wavefield[2:] - forward_wavefield[:-2]) / (2 * dt)
                adjoint_list = [np.array(adj) for adj in reverse_loop_gen][::-1]
                for k in range(1, steps-1):
                    adjoint_field = adjoint_list[k]
