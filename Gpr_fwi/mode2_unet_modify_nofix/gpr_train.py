...
from gradient import compute_gradient, compute_laplacian_2d, compute_tikhonov_gradient
...
if __name__ == "__main__":
    ...
    result_dir = f'result_gpr_lr{learning_rate:.0e}_alpha{alpha_tikhonov_eps:.3g}_offset{offset:.3g}/'
    ...
    #绘制epsilon_true
    plt.figure(figsize=(15, 5))
    plt.imshow(epsilon_true, aspect='auto', 
                          cmap='jet', vmin=0, vmax=10)
    plt.title(f'Epsilon_true')
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(f'{path}epsilon_true.png')
    plt.close()
    print(f"已保存epsilon_true图")
    # 转换为torch张量
    ...
    receiver_list = [(0, i) for i in range(10, zl-10, 5)]  # 接收点
    ...
    # 保存观测数据到文件
    np.save(f'{path}d_obs.npy', d_obs)
    print(f"已保存观测数据到 {path}d_obs.npy")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            # 输出net1_output_model的统计信息
            net1_output_np = net1_output_model.detach().cpu().numpy()
            print(f"net1_output_model 形状: {net1_output_np.shape}")
            print(f"net1_output_model 最小值: {net1_output_np.min():.6f}")
            print(f"net1_output_model 最大值: {net1_output_np.max():.6f}")
            print(f"net1_output_model 平均值: {net1_output_np.mean():.6f}")
            print(f"net1_output_model 标准差: {net1_output_np.std():.6f}")

            plt.figure(figsize=(8, 5))
            plt.hist(net1_output_np.flatten(), bins=50, color='skyblue', edgecolor='black')
            plt.title(f'net1_output_model 分布 (epoch {epoch}, batch {i})')
            plt.xlabel('值')
            plt.ylabel('频数')
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig(f"{path}epoch_{epoch}_batch_{i}_net1_output_hist.png", dpi=200)
            plt.close()
            print(f"已保存net1_output_model分布直方图: {path}epoch_{epoch}_batch_{i}_net1_output_hist.png")
            ...
            # 添加迁移项
            outputs_eps = net1_o + epsilon_mig
            # 保持浅层=真实模型
            ...
            d_syn = ForwardModelFunction.apply(
                outputs_eps, sigma_true, selected_source_list, receiver_list, 
            ...
            # 计算数据损失
            data_loss = loss_fn(d_syn, selected_d_obs.to(device))
