...
class ForwardModelFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        # 计算梯度
        grad_eps, grad_sig = compute_gradient(epsilon_np, sigma_np, grad_output_np, 
                                            source_list, receiver_list, dt, dx, dz, npml, 
                                            freq, steps, sigma_required_gradient=True, wavefield_data=wave_field)
...
def plot_loss_curves(loss_history, eps_loss_history, sig_loss_history, save_path, current_epoch):
    """绘制损失曲线"""
    ...
    # Sigma模型损失
    plt.subplot(1, 4, 4)
    plt.plot(sig_loss_history, 'm-', linewidth=2)
    plt.title('Sigma Model Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.grid(True, alpha=0.3)
...
def plot_true_models(epsilon_true, sigma_true, save_path):
    """绘制并保存真实模型图像"""
    print("绘制真实模型图像...")
    
    # 转换为numpy数组用于绘图
    epsilon_true_np = epsilon_true.cpu().numpy()
    sigma_true_np = sigma_true.cpu().numpy()
    
    # 创建图形
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    
    # 绘制epsilon真实模型
    im1 = axes[0].imshow(epsilon_true_np, aspect='auto', cmap='jet', vmin=0, vmax=10.0)
    axes[0].set_title('true permittivity (Epsilon True)', fontsize=14, fontweight='bold')
    axes[0].set_xlabel('x', fontsize=12)
    axes[0].set_ylabel('z', fontsize=12)
    cbar1 = plt.colorbar(im1, ax=axes[0])
    cbar1.set_label('permittivity', fontsize=12)
    
    # 绘制sigma真实模型
    im2 = axes[1].imshow(sigma_true_np, aspect='auto', cmap='jet', vmin=0, vmax=2e-3)
    axes[1].set_title('true conductivity (Sigma True)', fontsize=14, fontweight='bold')
    axes[1].set_xlabel('x', fontsize=12)
    axes[1].set_ylabel('z', fontsize=12)
    cbar2 = plt.colorbar(im2, ax=axes[1])
    cbar2.set_label('conductivity (S/m)', fontsize=12)
    
    # 调整布局
    plt.tight_layout()
    
    # 保存图像
    plt.savefig(f'{save_path}true_models.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    # 保存数据
    np.save(f'{save_path}epsilon_true.npy', epsilon_true_np)
    np.save(f'{save_path}sigma_true.npy', sigma_true_np)
    
    print(f"真实模型图像已保存到: {save_path}true_models.png")
    print(f"真实模型数据已保存到: {save_path}epsilon_true.npy 和 {save_path}sigma_true.npy")
    
    # 打印模型统计信息
    print(f"Epsilon真实模型统计:")
    print(f"  形状: {epsilon_true_np.shape}")
    print(f"  最小值: {np.min(epsilon_true_np):.3f}")
    print(f"  最大值: {np.max(epsilon_true_np):.3f}")
    print(f"  平均值: {np.mean(epsilon_true_np):.3f}")
    
    print(f"Sigma真实模型统计:")
    print(f"  形状: {sigma_true_np.shape}")
    print(f"  最小值: {np.min(sigma_true_np):.2e}")
    print(f"  最大值: {np.max(sigma_true_np):.2e}")
    print(f"  平均值: {np.mean(sigma_true_np):.2e}")
...
def main():
    ...
    alpha_tikhonov_sig = 1e-6  # 电导率Tikhonov正则化参数
    offset_eps = 6
    offset_sig = 6
    # 构造结果文件夹名，包含学习率和alpha_tikhonov_eps
    result_dir = f'result_gpr_lr{learning_rate:.0e}_alpha{alpha_tikhonov_eps:.3g}_sig{alpha_tikhonov_sig:.3g}_offset_eps{offset_eps:.3g}_offset_sig{offset_sig:.3g}/'
    ...
    # 源点和接收点设置
    source_list = [(0, i) for i in range(10, zl-10, 10)]  # 28个源点
    ...
    # ========== 绘制并保存真实模型 ==========
    plot_true_models(epsilon_true, sigma_true, path)
    ...
    d_obs = torch.tensor(d_obs, dtype=torch.float32, device=device)
    ...
    print(f"epsilon网络参数数量: {get_parameter_number(net_1)}")
    print(f"sigma网络参数数量: {get_parameter_number(net_2)}")
    ...
    net2_input = sig_input_padded.unsqueeze(0).unsqueeze(0).to(device)  # [1, 1, height+pad_height, width+pad_width]
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': net_2.parameters(), 'lr': learning_rate},
    ])
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            batch_net2_input = net2_input[:, i:i+1, :, :].to(device)
            ...
            net1_ = torch.sigmoid(net1_output_model+offset_eps)
            net2_ = torch.sigmoid(net2_output_model+offset_sig)
            # 缩放到物理范围
            ...
            # sigma = torch.cat((sigma_layer, net2_o), dim=0)  # 在深度维度连接
            ...
            outputs_sig = net2_o
            # outputs_sig[:l_size, :] = sigma_true[:l_size, :]
            outputs_sig = torch.clamp(outputs_sig, min=min_sig, max=max_sig)
            outputs_sig.retain_grad()
            
            # 使用自动微分正演模拟
            print(f"Epoch {epoch}, Batch {i}: 开始正演模拟...")
            d_syn = ForwardModelFunction.apply(
                outputs_eps, outputs_sig, selected_source_list, receiver_list, 
            ...
            # 计算正则化损失
            reg_loss = tikhonov_reg(outputs_eps, outputs_sig, alpha_tikhonov_eps, alpha_tikhonov_sig, dx, dz)
            ...
            if epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                ...
                # Sigma模型可视化
                plt.figure(figsize=(15, 5))
                plt.imshow(outputs_sig.detach().cpu().numpy(), aspect='auto', 
                          cmap='jet', vmin=0, vmax=2e-3)
                plt.title(f'Sigma (epoch {epoch})')
                plt.colorbar()
                plt.tight_layout()
                plt.savefig(f'{path}epoch_{epoch}_sigma.png')
                plt.close()
                ...
                print(f"Epoch {epoch}: outputs_sig.grad is None: {outputs_sig.grad is None}")
                print(f"Epoch {epoch}: outputs_sig.requires_grad: {outputs_sig.requires_grad}")
                
                # 可视化梯度
                if outputs_eps.grad is not None:
                    print(f"Epoch {epoch}: 开始绘制epsilon梯度...")
                    grad_eps_np = outputs_eps.grad.detach().cpu().numpy()
                    print(f"Epoch {epoch}: epsilon梯度形状: {grad_eps_np.shape}")
                    print(f"Epoch {epoch}: epsilon梯度范围: [{np.min(grad_eps_np):.6f}, {np.max(grad_eps_np):.6f}]")
                    
                    # Epsilon梯度图
                    plt.figure(figsize=(15, 5))
                    plt.imshow(grad_eps_np, aspect='auto', cmap='RdBu_r', 
                              vmin=-np.max(np.abs(grad_eps_np)), vmax=np.max(np.abs(grad_eps_np)))
                    plt.title(f'Epsilon Gradient (epoch {epoch})')
                    plt.colorbar()
                    plt.tight_layout()
                    plt.savefig(f'{path}epoch_{epoch}_epsilon_grad.png')
                    plt.close()
                    print(f"Epoch {epoch}: 已保存epsilon梯度图")
                    
                    # 保存epsilon梯度数据
                    np.save(f'{path}epoch_{epoch}_epsilon_grad.npy', grad_eps_np)
                    print(f"Epoch {epoch}: 已保存epsilon梯度数据")
                else:
                    print(f"Epoch {epoch}: epsilon梯度为None，无法绘制")
                
                # Sigma梯度可视化
                if outputs_sig.grad is not None:
                    print(f"Epoch {epoch}: 开始绘制sigma梯度...")
                    grad_sig_np = outputs_sig.grad.detach().cpu().numpy()
                    print(f"Epoch {epoch}: sigma梯度形状: {grad_sig_np.shape}")
                    print(f"Epoch {epoch}: sigma梯度范围: [{np.min(grad_sig_np):.6f}, {np.max(grad_sig_np):.6f}]")
                    
                    # Sigma梯度图
                    plt.figure(figsize=(15, 5))
                    plt.imshow(grad_sig_np, aspect='auto', cmap='RdBu_r', 
                              vmin=-np.max(np.abs(grad_sig_np)), vmax=np.max(np.abs(grad_sig_np)))
                    plt.title(f'Sigma Gradient (epoch {epoch})')
                    plt.colorbar()
                    plt.tight_layout()
                    plt.savefig(f'{path}epoch_{epoch}_sigma_grad.png')
                    plt.close()
                    print(f"Epoch {epoch}: 已保存sigma梯度图")
                    
                    # 保存sigma梯度数据
                    np.save(f'{path}epoch_{epoch}_sigma_grad.npy', grad_sig_np)
                    print(f"Epoch {epoch}: 已保存sigma梯度数据")
                else:
                    print(f"Epoch {epoch}: sigma梯度为None，无法绘制")
                
                # 保存数据
                np.save(f'{path}epoch_{epoch}_epsilon.npy', outputs_eps.detach().cpu().numpy())
                np.save(f'{path}epoch_{epoch}_sigma.npy', outputs_sig.detach().cpu().numpy())
                print(f"Epoch {epoch}: 已保存epsilon和sigma数据")
        ...
        loss_model_sig = loss_fn(outputs_sig, sigma_true) / loss_fn(sigma_true, torch.zeros_like(sigma_true))
        
        # 记录到列表用于绘图
        loss_history.append(epoch_loss)
        eps_loss_history.append(loss_model_eps.item())
        sig_loss_history.append(loss_model_sig.item())
        
        print(f'Epoch {epoch}: Data Loss = {epoch_loss:.6f}, Eps Loss = {loss_model_eps:.6f}, Sig Loss = {loss_model_sig:.6f}')
        
        # 每100个epoch绘制一次loss曲线
        if epoch % 100 == 0 and epoch > 0:
            plot_loss_curves(loss_history, eps_loss_history, sig_loss_history, path, epoch)
    
    # 训练结束后绘制最终的loss曲线
    print("绘制最终loss曲线...")
    plot_loss_curves(loss_history, eps_loss_history, sig_loss_history, path, num_epochs-1)
    ...
    np.save(f'{path}final_sig_loss_history.npy', np.array(sig_loss_history))
