...
if __name__ == "__main__":
    ...
    result_dir = f'result_gpr_lr{learning_rate:.0e}_alpha{alpha_tikhonov_eps:.3g}/'
    ...
    # # ====== 小测试：将epsilon_true padding后输入网络，查看输出分布直方图 ======
    # with torch.no_grad():
    #     # 对epsilon_true做padding
    #     eps_true_padded = F.pad(epsilon_true, (pad_width//2, pad_width//2, pad_height//2, pad_height//2))
    #     # 扩展维度
    #     eps_true_input = eps_true_padded.unsqueeze(0).unsqueeze(0).to(device)
    #     # 前向传播
    #     _, _, net1_output_true = net_1(eps_true_input)
    #     # 输出分布直方图
    #     net1_output_true_np = net1_output_true.detach().cpu().numpy().flatten()
    #     plt.figure()
    #     plt.hist(net1_output_true_np, bins=50, color='green', alpha=0.7)
    #     plt.title('网络输入真实epsilon_true时输出的数据分布直方图')
    #     plt.xlabel('值')
    #     plt.ylabel('频数')
    #     plt.grid(True, alpha=0.3)
    #     plt.tight_layout()
    #     plt.savefig('net1_output_true_hist.png')
    #     plt.close()
    #     print("已保存真实epsilon_true输入网络后的输出分布直方图: net1_output_true_hist.png")
    # 训练循环
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            # # 输出net1_output_model的数据分布直方图
            # net1_output_np = net1_output_model.detach().cpu().numpy().flatten()
            # plt.figure()
            # plt.hist(net1_output_np, bins=50, color='blue', alpha=0.7)
            # plt.title('net1_output_model 数据分布直方图')
            # plt.xlabel('值')
            # plt.ylabel('频数')
            # plt.grid(True, alpha=0.3)
            # plt.tight_layout()
            # plt.savefig(f'net1_output_hist.png')
            # plt.close()
            ...
            net1_ = torch.sigmoid(net1_output_model+4)
