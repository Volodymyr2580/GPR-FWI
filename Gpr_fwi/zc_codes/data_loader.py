...
def DataLoad_Train(n_shots):
    ...
    plt.figure()
    plt.imshow(v_true.cpu().T, aspect='auto', cmap='jet')
    plt.xlabel("X")
    plt.ylabel("Z")
    plt.title("Velocity")
    plt.colorbar()
    ...
    source_locations[:, 0, 0] = torch.arange(n_shots, device=device) * d_source + first_source
    ...
    receiver_locations[:, :, 0] = (
        (torch.arange(n_receivers_per_shot, device=device) * d_receiver + first_receiver)
            .repeat(n_shots, 1)
    )
    ...
    sos = [torch.tensor(sosi, device=device).to(source_amplitudes.dtype)
           for sosi in sos]
    ...
    observed_data = scalar(v_true, dx, dt,
                          source_amplitudes=source_amplitudes_filt,
    ...
    return observed_data, data_dim, model_dim, source_amplitudes_filt, source_locations, receiver_locations, v_true
