...
def main():
    ...
    def get_current_temperature(epoch):
        return T0*np.exp(-cooling_rate*np.sqrt(epoch))
    ...
    for epoch in range(epochs):
        ...
        plt.figure()
        plt.imshow(update_eps_grad.reshape(model_eps.shape), cmap='viridis')
        plt.title(f'epsilon update grad (epoch {epoch})')
        plt.colorbar()
        plt.savefig(f'results/update_eps_grad_epoch{epoch}.png')
        plt.close()
        ...
        plt.figure()
        plt.imshow(tikhonov_grad_eps, cmap='viridis')
        plt.title(f'epsilon tikhonov grad (epoch {epoch})')
        plt.colorbar()
        plt.savefig(f'results/tikhonov_grad_eps_epoch{epoch}.png')
        plt.close()
        ...
        # 生成一个与model_eps.flatten()相同维数的随机向量y，取值范围在0到1之间
        ...
        print(f"扰动更新量update_SA: max={np.max(update_SA)}, min={np.min(update_SA)}, mean={np.mean(update_SA)}")
        ...
        print('计算新模型loss')
        d_syn_new = forward_model(new_eps.reshape(model_eps.shape), model_sig, selected_source_list, receiver_list, dt, dx, dz, npml, freq, steps, save_wavefield=False)

        loss_new = np.linalg.norm((d_syn_new-selected_d_obs).flatten())
