...
import cupy as cp
import sys
from .Time_loop_cupy import reverse_time_loop
from .Add_CPML import Add_CPML
from .forward import WAVEFIELD_SAMPLE_INTERVAL
...
def compute_tikhonov_gradient(model, dx, dz, alpha_tikhonov=0.01):
    ...
    max_abs = cp.max(cp.abs(tikhonov_grad))
    if max_abs > 0:
...
def compute_gradient_mpi(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient=False, wavefield_data=None, global_start_idx=0):
    ...
    local_grad_eps = cp.zeros_like(epsilon)
    local_grad_sig = cp.zeros_like(sigma) if sigma_required_gradient else None
    ...
    for local_shot_idx, (source_pos, receiver_pos) in enumerate(zip(local_source_list, local_receiver_list)):
        ...
        reverse_loop_gen = reverse_time_loop(
            ...
            residual_local[local_shot_idx, :],
        )
        
        if wavefield_data is not None:
            
            forward_wavefield = cp.stack(wavefield_data[local_shot_idx], axis=0)

            

            dt_sampled = dt * WAVEFIELD_SAMPLE_INTERVAL
            time_weight = WAVEFIELD_SAMPLE_INTERVAL
            forward_diff = (forward_wavefield[2:] - forward_wavefield[:-2]) / (2 * dt_sampled)

            adjoint_snapshots = []
            for step_idx, adj in enumerate(reverse_loop_gen):
                if step_idx % WAVEFIELD_SAMPLE_INTERVAL == 0:
                    adjoint_snapshots.append(adj)
            adjoint_list = adjoint_snapshots[::-1]

            n_snapshots = len(adjoint_list)

            for sample_idx in range(1, n_snapshots - 1):
                adjoint_field = adjoint_list[sample_idx]
                forward_term = forward_diff[sample_idx - 1]
                local_grad_eps += (
                        ep0
                        * time_weight
                        * adjoint_field[npml:npml + xl, npml:npml + zl]
                        * forward_term[npml:npml + xl, npml:npml + zl]
                    )
                if sigma_required_gradient:
                        local_grad_sig += (
                            time_weight
                            * adjoint_field[npml:npml + xl, npml:npml + zl]
                            * forward_wavefield[sample_idx][npml:npml + xl, npml:npml + zl]
                        )
            
    
    # 收集所有进程的梯度
    all_grad_eps = comm.gather(cp.asnumpy(local_grad_eps), root=0)
    if sigma_required_gradient:
        all_grad_sig = comm.gather(cp.asnumpy(local_grad_sig), root=0)
    ...
    if sigma_required_gradient:
        ...
        return cp.asarray(grad_eps), cp.asarray(grad_sig)
    else:
        return cp.asarray(grad_eps), None

def compute_gradient(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient=False, wavefield_data=None, force_serial=False):
    """
    ...
    if (not force_serial) and MPI_AVAILABLE and MPI.COMM_WORLD.Get_size() > 1:
        return compute_gradient_mpi(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient, wavefield_data, force_serial)
    ...
    n_shots = len(source_list)
    assert n_shots == len(receiver_list)
    grad_eps = cp.zeros_like(epsilon)
    grad_sig = cp.zeros_like(sigma) if sigma_required_gradient else None
    ep0 = 8.841941282883074e-12
    for shot_idx, (source_pos, receiver_pos) in enumerate(zip(source_list, receiver_list)):
        ...
        if wavefield_data is not None:
            ...
            if shot_idx < len(wavefield_data) and len(wavefield_data[shot_idx]) > 0:
                forward_wavefield = cp.stack(wavefield_data[shot_idx], axis=0)
