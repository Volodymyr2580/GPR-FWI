...
class ForwardModelFunction(Function):
    ...
    def forward(
        ...
        data_tensor = torch_dlpack.from_dlpack(data).to(epsilon.device).float()
    ...
    def backward(ctx, grad_output):
        ...
        epsilon_np = epsilon.detach().cpu().numpy()
        sigma_np = sigma.detach().cpu().numpy()
        grad_output_np = cp.from_dlpack(grad_output.to(epsilon.device))
        ...
        # 转换回torch张量
        grad_eps_tensor = torch_dlpack.from_dlpack(grad_eps).to(epsilon.device)
        grad_sig_tensor = (
            torch_dlpack.from_dlpack(grad_sig).to(sigma.device) if grad_sig is not None else None
        )
...
def main():
    ...
    for shot_idx in local_shots:
        ...
        local_d_obs_list.append(cp.asnumpy(d_obs_shot[0]))
