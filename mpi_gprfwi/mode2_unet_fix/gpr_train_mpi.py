...
def visualize_data_dimensions_mpi(d_syn, d_obs, source_list, receiver_list, save_path, epoch, batch_idx, rank):
    """MPI版本的数据维度可视化函数，帮助调试新的数据结构"""
    if rank != 0:  # 只在主进程执行可视化
        return
    ...
    plt.subplot(3, 4, 1)
    plt.axis('off')
    info_text = f"""Data Dimensions (Epoch {epoch}, Batch {batch_idx}):
...
Source List: {len(source_list)} sources
Receiver List: {len(receiver_list)} receivers"""
    ...
    # 第一个炮点的数据
    plt.subplot(3, 4, 2)
    plt.imshow(d_syn[0, :, :].detach().cpu().numpy().T, aspect='auto', cmap='seismic')
    plt.title(f'Synthetic - Shot 0')
    plt.xlabel('Receiver Index')
    plt.ylabel('Time Step')
    plt.colorbar()
    
    plt.subplot(3, 4, 3)
    plt.imshow(d_obs[0, :, :].detach().cpu().numpy().T, aspect='auto', cmap='seismic')
    plt.title(f'Observed - Shot 0')
    plt.xlabel('Receiver Index')
    plt.ylabel('Time Step')
    plt.colorbar()
    
    # 残差
    residual = d_syn[0, :, :].detach().cpu().numpy() - d_obs[0, :, :].detach().cpu().numpy()
    plt.subplot(3, 4, 4)
    plt.imshow(residual.T, aspect='auto', cmap='RdBu_r', 
               vmin=-np.max(np.abs(residual)), vmax=np.max(np.abs(residual)))
    plt.title(f'Residual - Shot 0')
    plt.xlabel('Receiver Index')
    plt.ylabel('Time Step')
    plt.colorbar()
    
    # 时间切片
    time_slices = [d_syn.shape[2]//4, d_syn.shape[2]//2, 3*d_syn.shape[2]//4]
    for i, t_idx in enumerate(time_slices):
        plt.subplot(3, 4, 5+i)
        plt.plot(d_syn[0, :, t_idx].detach().cpu().numpy(), 'b-', label='Synthetic', linewidth=2)
        plt.plot(d_obs[0, :, t_idx].detach().cpu().numpy(), 'r--', label='Observed', linewidth=2)
        plt.title(f'Time Slice {t_idx}')
        plt.xlabel('Receiver Index')
    ...
    # 接收器切片
    rec_slices = [d_syn.shape[1]//4, d_syn.shape[1]//2, 3*d_syn.shape[1]//4]
    for i, r_idx in enumerate(rec_slices):
        plt.subplot(3, 4, 8+i)
        plt.plot(d_syn[0, r_idx, :].detach().cpu().numpy(), 'b-', label='Synthetic', linewidth=2)
        plt.plot(d_obs[0, r_idx, :].detach().cpu().numpy(), 'r--', label='Observed', linewidth=2)
        plt.title(f'Receiver {r_idx}')
    ...
    plt.tight_layout()
    plt.savefig(f'{save_path}epoch_{epoch}_batch_{batch_idx}_data_dimensions_mpi.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"已保存MPI数据维度可视化图: {save_path}epoch_{epoch}_batch_{batch_idx}_data_dimensions_mpi.png")
...
def main():
    ...
    if rank == 0:
        ...
        if d_obs.dim() == 3:  # (n_shots, n_receivers, n_steps)
            ...
            plt.subplot(1, 3, 1)
            plt.imshow(d_obs[0, :, :].detach().cpu().numpy().T, aspect='auto', cmap='seismic')
            plt.title(f'Shot 0 Data (n_receivers={d_obs.shape[1]}, n_steps={d_obs.shape[2]})')
            plt.xlabel('Receiver Index')
            plt.ylabel('Time Step')
            plt.colorbar()
            ...
            plt.subplot(1, 3, 2)
            plt.imshow(d_obs[:, 0, :].detach().cpu().numpy().T, aspect='auto', cmap='seismic')
            plt.title(f'Receiver 0 Data (n_shots={d_obs.shape[0]}, n_steps={d_obs.shape[2]})')
            ...
            # 时间切片数据
            plt.subplot(1, 3, 3)
            plt.imshow(d_obs[:, :, d_obs.shape[2]//2].detach().cpu().numpy(), aspect='auto', cmap='seismic')
            plt.title(f'Time Slice {d_obs.shape[2]//2} (n_shots={d_obs.shape[0]}, n_receivers={d_obs.shape[1]})')
            plt.xlabel('Receiver Index')
            plt.ylabel('Shot Index')
            plt.colorbar()
            
            plt.tight_layout()
            plt.savefig(f'{path}data_structure_visualization.png', dpi=300, bbox_inches='tight')
            plt.close()
            print(f"已保存数据结构可视化图: {path}data_structure_visualization.png")
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            if rank == 0:
                ...
                if epoch < 5 and i == 0:
                    ...
                    if d_syn.dim() == 3 and selected_d_obs_local.dim() == 3:
                        visualize_data_dimensions_mpi(d_syn, selected_d_obs_local, selected_source_list, receiver_list, path, epoch, i, rank)
            ...
            if not first_iter_saved and epoch == 0 and i == 0:
                ...
                if rank == 0:
                    ...
                    # 处理3D数据结构的可视化
                    if d_syn_global.ndim == 3:  # (n_shots, n_receivers, n_steps)
                        # 第一个炮点的数据
                        plt.figure(figsize=(15, 5))
                        
                        plt.subplot(1, 3, 1)
                        plt.imshow(d_syn_global[0, :, :].T, aspect='auto', cmap='seismic')
                        plt.title(f'First Iteration - Shot 0 (n_receivers={d_syn_global.shape[1]}, n_steps={d_syn_global.shape[2]})')
                        plt.xlabel('Receiver Index')
                        plt.ylabel('Time Step')
                        plt.colorbar()
                        ...
                        plt.subplot(1, 3, 2)
                        plt.imshow(d_syn_global[:, 0, :].T, aspect='auto', cmap='seismic')
                        plt.title(f'First Iteration - Receiver 0 (n_shots={d_syn_global.shape[0]}, n_steps={d_syn_global.shape[2]})')
                        plt.xlabel('Shot Index')
                        plt.ylabel('Time Step')
                        plt.colorbar()
                        
                        plt.subplot(1, 3, 3)
                        plt.imshow(d_syn_global[:, :, d_syn_global.shape[2]//2], aspect='auto', cmap='seismic')
                        plt.title(f'First Iteration - Time Slice {d_syn_global.shape[2]//2}')
                        plt.xlabel('Receiver Index')
                        plt.ylabel('Shot Index')
                        plt.colorbar()
                        
                        plt.tight_layout()
                        plt.savefig(f"{path}d_syn_first_3d.png", dpi=300, bbox_inches='tight')
                        plt.close()
                        ...
                        plt.subplot(1, 3, 1)
                        plt.imshow(residual[0, :, :].T, aspect='auto', cmap='RdBu_r', 
                                  vmin=-np.max(np.abs(residual)), vmax=np.max(np.abs(residual)))
                        plt.title(f'First Iteration Residual - Shot 0')
                        plt.xlabel('Receiver Index')
                        plt.ylabel('Time Step')
                        plt.colorbar()
                        ...
                        plt.imshow(residual[:, 0, :].T, aspect='auto', cmap='RdBu_r', 
                                  vmin=-np.max(np.abs(residual)), vmax=np.max(np.abs(residual)))
                        plt.title(f'First Iteration Residual - Receiver 0')
                        plt.xlabel('Shot Index')
                        plt.ylabel('Time Step')
                        plt.colorbar()
                        
                        plt.subplot(1, 3, 3)
                        plt.imshow(residual[:, :, residual.shape[2]//2], aspect='auto', cmap='RdBu_r', 
                                  vmin=-np.max(np.abs(residual)), vmax=np.max(np.abs(residual)))
                        plt.title(f'First Iteration Residual - Time Slice {residual.shape[2]//2}')
                        plt.xlabel('Receiver Index')
                        plt.ylabel('Shot Index')
                        plt.colorbar()
                        
                        plt.tight_layout()
                        plt.savefig(f"{path}residual_first_3d.png", dpi=300, bbox_inches='tight')
                        plt.close()
                        ...
                        plt.title('First Iteration Synthetic Data (d_syn) - 2D Format')
                        plt.colorbar(); plt.tight_layout()
                        ...
                        plt.title('First Iteration Residual (d_syn - d_obs) - 2D Format')
                        plt.colorbar(); plt.tight_layout()
            ...
            if rank == 0:
                ...
                if epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                    # 处理3D数据结构的可视化
                    if d_syn.dim() == 3:  # (n_shots, n_receivers, n_steps)
                        # 第一个炮点的数据
                        plt.figure(figsize=(15, 5))
                        
                        plt.subplot(1, 3, 1)
                        plt.imshow(d_syn[0, :, :].detach().cpu().numpy().T, aspect='auto', cmap='seismic')
                        plt.title(f'Synthetic Data - Shot 0 (epoch {epoch})')
                        plt.xlabel('Receiver Index')
                        plt.ylabel('Time Step')
                        plt.colorbar()
                        ...
                        plt.subplot(1, 3, 2)
                        plt.imshow(d_syn[:, 0, :].detach().cpu().numpy().T, aspect='auto', cmap='seismic')
                        plt.title(f'Synthetic Data - Receiver 0 (epoch {epoch})')
                        plt.xlabel('Shot Index')
                        plt.ylabel('Time Step')
                        plt.colorbar()
                        
                        plt.subplot(1, 3, 3)
                        plt.imshow(d_syn[:, :, d_syn.shape[2]//2].detach().cpu().numpy(), aspect='auto', cmap='seismic')
                        plt.title(f'Synthetic Data - Time Slice {d_syn.shape[2]//2} (epoch {epoch})')
                        plt.xlabel('Receiver Index')
                        plt.ylabel('Shot Index')
                        plt.colorbar()
                        
                        plt.tight_layout()
                        plt.savefig(f"{path}epoch_{epoch}_d_syn_3d.png", dpi=300, bbox_inches='tight')
                        plt.close()
                    else:
                        # 兼容旧的2D数据格式
                        plt.figure(figsize=(12, 4))
                        plt.imshow(d_syn.detach().cpu().numpy().T, aspect='auto', cmap='seismic')
                        plt.title(f'Synthetic Data (epoch {epoch}) - 2D Format')
                        plt.colorbar(); plt.tight_layout()
