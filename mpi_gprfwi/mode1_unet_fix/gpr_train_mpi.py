...
def main():
    ...
    if rank == 0:
        ...
        plt.imshow(d_obs.detach().cpu().numpy().T, aspect='auto', cmap='seismic', vmin=-10, vmax=10)
        plt.title('Observed Data (d_obs)')
        plt.colorbar(); plt.tight_layout()
        plt.savefig(f"{path}d_obs.png"); plt.close()

        np.save(f'{path}d_obs_mode1.npy', d_obs_np)
        print('观测数据已经保存')
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            if not first_iter_saved and epoch == 0 and i == 0:
                ...
                if rank == 0:
                    ...
                    plt.figure(figsize=(12, 4))
                    plt.imshow(d_syn_global.T, aspect='auto', cmap='seismic', vmin=-10, vmax=10)
