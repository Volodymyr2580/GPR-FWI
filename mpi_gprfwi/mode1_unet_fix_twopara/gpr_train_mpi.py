...
def main():
    ...
    learning_rate_sig = 1e-3  # 学习率
    alpha_tikhonov_eps = 0  # Tikhonov正则化参数
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            if rank == 0 and net_1 is not None:
                ...
                #
                # 构造电导率模型，与epsilon相同流程（浅层固定 + 迁移项 + 物理范围约束）
                #
                max_value = net2_output_model.max().item()
                min_value = net2_output_model.min().item()
                mean_value = net2_output_model.mean().item()
                print(f"net2_output_model 统计量: 最大值={max_value}, 最小值={min_value}, 平均值={mean_value}")

                net2_o = torch.sigmoid(net2_output_model + offset)
                # 缩放到物理范围
                net2_o = net2_o * (max_sig - min_sig) + min_sig
                # 连接浅层固定模型和网络输出
                epsilon = torch.cat((epsilon_layer, net1_o), dim=0)
                sigma = torch.cat((sigma_layer, net2_o), dim=0)
                ...
                # 对sigma执行相同的流程
                outputs_sig_root = sigma + sigma_mig
                outputs_sig_root[:l_size, :] = sigma_true[:l_size, :]
                outputs_sig_root = torch.clamp(outputs_sig_root, min=min_sig, max=max_sig)
            ...
            if rank == 0 and net_2 is not None:
                outputs_sig_use = outputs_sig_root
            else:
                outputs_sig_use = torch.tensor(outputs_sig_np, dtype=torch.float32, device=device, requires_grad=True)
            ...
            reg_loss = tikhonov_reg(outputs_eps_use, outputs_sig_use, alpha_tikhonov_eps, alpha_tikhonov_sig, dx, dz)
            ...
            if rank == 0:
                ...
                if epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                    ...
                    plt.figure(figsize=(15, 5))
                    plt.imshow(outputs_sig_use.detach().cpu().numpy(), aspect='auto', 
                              cmap='jet', vmin=1e-5, vmax=1e-3)
                    ...
                    print(f"Epoch {epoch}: outputs_sig.grad is None: {outputs_sig_use.grad is None}")
                    print(f"Epoch {epoch}: outputs_sig.requires_grad: {outputs_sig_use.requires_grad}")
                    ...
                    if outputs_sig_use.grad is not None:
                        ...
                        sig_grad_np = outputs_sig_use.grad.detach().cpu().numpy()
                        print(f"Epoch {epoch}: sigma梯度形状: {sig_grad_np.shape}")
                        print(f"Epoch {epoch}: sigma梯度范围: [{np.min(sig_grad_np):.6f}, {np.max(sig_grad_np):.6f}]")
                        
                        # 归一化sigma梯度
                        max_abs_sig_grad = np.max(np.abs(sig_grad_np))
                        if max_abs_sig_grad != 0:
                            sig_grad_np_norm = sig_grad_np / max_abs_sig_grad
                        else:
                            sig_grad_np_norm = sig_grad_np.copy()
                        print(f"Epoch {epoch}: sigma归一化后梯度范围: [{np.min(sig_grad_np_norm):.6f}, {np.max(sig_grad_np_norm):.6f}]")
                        
                        # sigma梯度图（归一化后）
                        plt.figure(figsize=(15, 5))
                        plt.imshow(sig_grad_np_norm, aspect='auto', cmap='RdBu_r', 
                                  vmin=-1, vmax=1)
                        plt.title(f'Sigma Gradient (Normed, epoch {epoch})')
                        plt.colorbar()
                        plt.tight_layout()
                        ...
                        # 保存sigma归一化梯度数据
                        #np.save(f'{path}epoch_{epoch}_sigma_grad.npy', sig_grad_np_norm)
                        print(f"Epoch {epoch}: 已保存sigma归一化梯度数据")
                    else:
        ...
        if rank == 0 and total_samples > 0:
            ...
            print(f'Epoch {epoch}: Total Loss = {epoch_loss:.6f}, Eps Loss = {loss_model_eps:.6f}, Sig Loss = {loss_model_sig:.6f}')
