...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            if rank == 0 and net_1 is not None:
                # 在2950-3001 epochs中启用调试模式
                if 2950 <= epoch <= 3001:
                    net_1.set_debug_mode(True)
                    print(f"Epoch {epoch}: 启用UNet调试模式")
                elif epoch > 3001:
                    # 在3001 epoch后关闭调试模式
                    net_1.set_debug_mode(False)
                    if epoch == 3002:
                        print(f"Epoch {epoch}: 关闭UNet调试模式")
            ...
            if rank == 0:
                ...
                # 在2950-3001 epochs中输出详细信息
                if 2950 <= epoch <= 3001:
                    print(f"\n=== Epoch {epoch} 详细输出信息 ===")
                    
                    # 1. 输出epsilon信息
                    epsilon_np = outputs_eps_use.detach().cpu().numpy()
                    print(f"1. Epsilon输出信息:")
                    print(f"   形状: {epsilon_np.shape}")
                    print(f"   范围: [{np.min(epsilon_np):.6f}, {np.max(epsilon_np):.6f}]")
                    print(f"   平均值: {np.mean(epsilon_np):.6f}")
                    print(f"   标准差: {np.std(epsilon_np):.6f}")
                    
                    # 保存epsilon图像
                    plt.figure(figsize=(12, 8))
                    plt.imshow(epsilon_np, aspect='auto', cmap='jet', vmin=min_eps, vmax=max_eps)
                    plt.title(f'Epsilon Output (epoch {epoch})')
                    plt.colorbar()
                    plt.tight_layout()
                    plt.savefig(f'{path}epoch_{epoch}_epsilon_detailed.png', dpi=300, bbox_inches='tight')
                    plt.close()
                    print(f"   已保存epsilon图像: epoch_{epoch}_epsilon_detailed.png")
                    
                    # 保存epsilon数据
                    np.save(f'{path}epoch_{epoch}_epsilon.npy', epsilon_np)
                    print(f"   已保存epsilon数据: epoch_{epoch}_epsilon.npy")
                    
                    # 2. 输出epsilon梯度信息
                    print(f"\n2. Epsilon梯度信息:")
                    if outputs_eps_use.grad is not None:
                        grad_np = outputs_eps_use.grad.detach().cpu().numpy()
                        print(f"   梯度形状: {grad_np.shape}")
                        print(f"   梯度范围: [{np.min(grad_np):.6f}, {np.max(grad_np):.6f}]")
                        print(f"   梯度平均值: {np.mean(grad_np):.6f}")
                        print(f"   梯度标准差: {np.std(grad_np):.6f}")
                        print(f"   梯度L2范数: {np.linalg.norm(grad_np):.6f}")
                        ...
                        # 保存梯度图像
                        plt.figure(figsize=(12, 8))
                        plt.imshow(grad_np_norm, aspect='auto', cmap='RdBu_r', vmin=-1, vmax=1)
                        plt.title(f'Epsilon Gradient (Normalized, epoch {epoch})')
                        plt.colorbar()
                        plt.tight_layout()
                        plt.savefig(f'{path}epoch_{epoch}_epsilon_grad_detailed.png', dpi=300, bbox_inches='tight')
                        plt.close()
                        print(f"   已保存梯度图像: epoch_{epoch}_epsilon_grad_detailed.png")
                        
                        # 保存梯度数据
                        np.save(f'{path}epoch_{epoch}_epsilon_grad.npy', grad_np)
                        np.save(f'{path}epoch_{epoch}_epsilon_grad_norm.npy', grad_np_norm)
                        print(f"   已保存梯度数据: epoch_{epoch}_epsilon_grad.npy")
                    else:
                        print(f"   梯度为None，无法输出梯度信息")
                    
                    # 3. 输出UNet网络每层信息
                    print(f"\n3. UNet网络层信息:")
                    if hasattr(net_1, 'debug_info') and net_1.debug_info:
                        # 保存UNet调试信息到文件
                        debug_info_file = f'{path}epoch_{epoch}_unet_debug_info.txt'
                        with open(debug_info_file, 'w', encoding='utf-8') as f:
                            f.write(f"UNet网络调试信息 - Epoch {epoch}\n")
                            f.write("=" * 50 + "\n\n")
                            
                            for layer_name, info in net_1.debug_info.items():
                        ...
                        print(f"   已保存UNet调试信息: epoch_{epoch}_unet_debug_info.txt")
                        
                        # 输出关键层信息到控制台
                        print(f"   输入层信息:")
                        if 'input' in net_1.debug_info:
                            input_info = net_1.debug_info['input']
                            print(f"     形状: {input_info['shape']}")
                            print(f"     范围: [{input_info['min']:.6f}, {input_info['max']:.6f}]")
                            print(f"     平均值: {input_info['mean']:.6f}")
                        
                        print(f"   编码器层信息:")
                        for i in range(5):  # 假设有5个编码器层
                            layer_name = f'encoder_{i}'
                            if layer_name in net_1.debug_info:
                                layer_info = net_1.debug_info[layer_name]
                                print(f"     {layer_name}: 形状={layer_info['output_shape']}, 范围=[{layer_info['output_min']:.6f}, {layer_info['output_max']:.6f}]")
                        
                        print(f"   解码器层信息:")
                        for i in range(5):  # 假设有5个解码器层
                        ...
                        print(f"   最终输出信息:")
                        if 'final_output' in net_1.debug_info:
                            final_info = net_1.debug_info['final_output']
                            print(f"     形状: {final_info['shape']}")
                            print(f"     范围: [{final_info['min']:.6f}, {final_info['max']:.6f}]")
                            print(f"     平均值: {final_info['mean']:.6f}")
                        
                        # 4. 可视化UNet每层输出
                        print(f"\n4. 可视化UNet每层输出:")
                        if hasattr(net_1, 'layer_outputs') and net_1.layer_outputs:
                            try:
                                # 可视化每层特征图
                                print(f"   正在生成每层特征图...")
                                visualize_unet_layers(net_1.layer_outputs, path, epoch)
                                
                                # 可视化特征统计信息
                                print(f"   正在生成特征统计图...")
                                visualize_feature_statistics(net_1.layer_outputs, path, epoch)
                                
                                # 创建层对比网格
                                print(f"   正在生成层对比图...")
                                create_layer_comparison_grid(net_1.layer_outputs, path, epoch)
                                
                                print(f"   已保存所有UNet层可视化图像")
                            except Exception as e:
                                print(f"   可视化过程中出现错误: {e}")
                        else:
                            print(f"   未获取到UNet层输出数据")
                    else:
                        print(f"   未获取到UNet调试信息")
                    
                    # 5. 输出网络参数更新信息
                    print(f"\n5. 网络参数更新信息:")
                    if net_1 is not None:
                        total_params = sum(p.numel() for p in net_1.parameters())
                        trainable_params = sum(p.numel() for p in net_1.parameters() if p.requires_grad)
                        ...
                        for name, param in net_1.named_parameters():
                            if param.grad is not None:
                                ...
                                param_updates.append((name, update_norm))
                        
                        if param_updates:
                            print(f"   参数更新量 (梯度L2范数):")
                            for name, update_norm in param_updates[:10]:  # 只显示前10个
                                print(f"     {name}: {update_norm:.6f}")
                            if len(param_updates) > 10:
                                print(f"     ... 还有 {len(param_updates) - 10} 个参数")
                        
                        # 6. 可视化梯度流
                        print(f"\n6. 可视化梯度流:")
                        try:
                            print(f"   正在生成梯度流图...")
                            visualize_gradient_flow(net_1, path, epoch)
                            print(f"   已保存梯度流可视化图像")
                        except Exception as e:
                            print(f"   梯度流可视化过程中出现错误: {e}")
                    
                    print(f"=== Epoch {epoch} 详细输出信息结束 ===\n")
                
                # 原有的可视化逻辑（保持兼容性）
                elif epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                    # 可视化结果
                    plt.figure(figsize=(15, 5))
