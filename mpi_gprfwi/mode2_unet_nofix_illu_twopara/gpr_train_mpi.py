...
class ForwardModelFunction(Function):
    ...
    def forward(ctx, epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps):
        ...
        if wave_field is not None:
            for shot_idx in range(len(wave_field)):
                ...
                # 输出forward_wavefield_valid的取值范围
                print(f"forward_wavefield_valid 取值范围: [{forward_wavefield_valid.min():.6e}, {forward_wavefield_valid.max():.6e}]")
    ...
    def backward(ctx, grad_output, grad_H):
        """
        反向传播：计算梯度
        grad_H: H的梯度（通常为None，因为我们不需要对H求导）
        """
        ...
        # 返回与forward函数输入参数数量相同的梯度
        # forward函数有10个参数：epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps
...
def main():
    ...
    # ========== 训练设置 ==========
    learning_rate_eps = 1e-3  # 学习率
    learning_rate_sig = 1e-3  # 学习率
    alpha_tikhonov_eps = 0.015  # Tikhonov正则化参数
    alpha_tikhonov_sig = 1e-5
    offset = 0
    
    # 构造结果文件夹名
    if rank == 0:
        result_dir = os.path.join(base_output_dir, f'lr_eps{learning_rate_eps:.0e}_lr_sig{learning_rate_sig:.0e}_alpha{alpha_tikhonov_eps:.3g}_offset{offset}')
    ...
    if rank == 0:
        ...
        # 额外保存：真实模型 sigma_true 图像
        plt.figure(figsize=(6, 4))
        plt.imshow(sigma_true.detach().cpu().numpy(), aspect='auto', cmap='jet', vmin=min_sig, vmax=max_sig)
    ...
    if rank == 0:
        optimizer = torch.optim.Adam([
            {'params': net_1.parameters(), 'lr': learning_rate_eps},
    ...
    for epoch in range(num_epochs):
        ...
        for i, (s_d_i, x_s_i, x_r_i) in enumerate(train_loader):
            ...
            if rank == 0 and net_1 is not None:
                ...
                net1_ = torch.sigmoid(net1_output_model)
                # 缩放到物理范围
                ...
                net2_ = torch.sigmoid(net2_output_model)
                net2_o = net2_ * (max_sig - min_sig) + min_sig
                outputs_sig_root = net2_o
            ...
            if rank == 0:
                ...
                if epoch % 50 == 0 or epoch in [1, 2, 3, 5, 10, 50, 100]:
                    ...
                    plt.imshow(outputs_sig_use.detach().cpu().numpy(), aspect='auto', 
                              cmap='jet', vmin=min_sig, vmax=max_sig)
                    ...
                    if outputs_sig_use.grad is not None:
                        print(f"Epoch {epoch}: 开始绘制梯度...")
                        grad_np = outputs_sig_use.grad.detach().cpu().numpy()
                        print(f"Epoch {epoch}: 梯度形状: {grad_np.shape}")
                        print(f"Epoch {epoch}: 梯度范围: [{np.min(grad_np):.6f}, {np.max(grad_np):.6f}]")
