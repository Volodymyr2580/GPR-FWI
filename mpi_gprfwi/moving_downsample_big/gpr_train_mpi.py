...
class ForwardModelFunction(Function):
    ...
    def forward(
        ...
        epsilon_np = np.ascontiguousarray(epsilon.detach().cpu().numpy(), dtype=np.float32)
        sigma_np = np.ascontiguousarray(sigma.detach().cpu().numpy(), dtype=np.float32)
...
STEPS = max(int(np.ceil(TIME_WINDOW / DT)), 1000)
...
def eps_to_sig(epsilon_r, freq=FREQ):
    ...
    index = 0.440 * np.log(epsilon) / np.log(1.93) - 2.943
...
def generate_ricker_wavelet(frequency, dt, steps):
    ...
    t = np.arange(steps, dtype=np.float32) * np.float32(dt)
...
def main():
    ...
    epsilon_true_np, epsilon_init_np = generate_models()
    # sigma_true_np = eps_to_sig(epsilon_true_np, freq=FREQ)
    sigma_true_np = np.ones_like(epsilon_true_np) * 0.005
    ...
    epsilon_true_pad_np = np.pad(epsilon_true_np, ((0, 0), (PADDING_WIDTH, PADDING_WIDTH)), mode='edge')
    sigma_true_pad_np = np.pad(sigma_true_np, ((0, 0), (PADDING_WIDTH, PADDING_WIDTH)), mode='edge')
    epsilon_init_pad_np = np.pad(epsilon_init_np, ((0, 0), (PADDING_WIDTH, PADDING_WIDTH)), mode='edge')
    ...
    if rank == 0:
        ...
        plt.imshow(epsilon_true_np, aspect='auto', cmap='jet', vmin=MIN_EPS, vmax=MAX_EPS)
    ...
    if rank == 0:
        ...
        plt.title('Ricker Wavelet (1.5e8 Hz)')
        plt.xlabel('Time Step')
        plt.ylabel('Amplitude')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f'{path}wavelet_ricker.png')
        plt.close()
        np.save(f'{path}wavelet_ricker.npy', wavelet_np)
    else:
        wavelet_np = None
    ...
    local_d_obs_list = []
    comm.Barrier()
    t_gen_local_start = time.perf_counter()
    BATCH_SIZE = 8
    for b_start in range(0, len(local_shots), BATCH_SIZE):
        b_end = min(b_start + BATCH_SIZE, len(local_shots))
        for shot_idx in local_shots[b_start:b_end]:
            ...
            epsilon_window_np = np.ascontiguousarray(epsilon_window_np, dtype=np.float32)
            sigma_window_np = np.ascontiguousarray(sigma_window_np, dtype=np.float32)
            ...
            d_obs_shot = forward_model(
                ...
                wavelet=np.ascontiguousarray(wavelet_np, dtype=np.float32),
                force_serial=True,
            )
            local_d_obs_list.append(d_obs_shot[0])
    t_gen_local_end = time.perf_counter()
    local_time_spent = t_gen_local_end - t_gen_local_start
    ...
    times_pieces = comm.gather(local_time_spent, root=0)
    if rank == 0:
        if d_obs_pieces:
        ...
        if times_pieces:
            total_gen_time = float(np.max(times_pieces))
            print(f"观测数据生成耗时(并行总耗时): {total_gen_time:.2f}s, 各进程耗时: {[round(t,2) for t in times_pieces]}")
    ...
    num_epochs = 10001  # 调试阶段仅运行1个epoch
    loss_history = []
    eps_loss_history = []
    ...
    for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                if fix_layer > 0:
                    ...
                    epsilon_big_eval[:fix_layer, :] = torch.tensor(
                        epsilon_true_np[:fix_layer, :], dtype=torch.float32, device=device
                    )
                else:
                    epsilon_big_eval = pred_eval_eps
            if epoch % VIS_SAVE_EVERY == 0:
                plt.figure(figsize=(12, 4))
        ...
        if rank == 0:
            ...
            if flat_records:
                ...
                for rec in flat_records:
                    grad_window = rec["grad_window"]
                    shot_idx = rec["shot_idx"]
                    plt.figure(figsize=(6, 4))
                    plt.imshow(grad_window, aspect='auto', cmap='seismic')
                    plt.title(f'Gradient Window Shot {shot_idx} (epoch {epoch})')
                    plt.colorbar()
                    plt.tight_layout()
                    plt.savefig(f"{path}epoch_{epoch:04d}_shot_{shot_idx:04d}_grad.png")
                    plt.close()
                ...
                for rec in flat_records:
                    ...
                    plt.plot(time_axis, rec["d_obs"], label='d_obs', linewidth=1.2)
                    plt.plot(time_axis, rec["d_syn"], label='d_syn', linewidth=1.0, alpha=0.8)
                    plt.plot(time_axis, rec["residual"], label='residual', linewidth=1.0, alpha=0.8)
                    plt.title(f'Time Series Shot {shot_idx} (epoch {epoch})')
                    plt.xlabel('Time Step')
                    plt.ylabel('Amplitude')
                    plt.legend(loc='upper right')
                    plt.grid(True, alpha=0.3)
                    plt.tight_layout()
                    plt.savefig(f"{path}epoch_{epoch:04d}_shot_{shot_idx:04d}_timeseries.png")
                    plt.close()
        '''
        ...
        if rank == 0 and net_1 is not None:
            ...
            if epoch % 1 == 0:
                ...
                plt.figure(figsize=(12, 4))
                plt.imshow(epsilon_big_eval.detach().cpu().numpy(), aspect='auto', cmap='jet', vmin=MIN_EPS, vmax=MAX_EPS)
                ...
                plt.figure(figsize=(12, 4))
                plt.imshow(grad_eps_no_pad.detach().cpu().numpy(), aspect='auto', cmap='seismic', vmin=-1, vmax=1)
