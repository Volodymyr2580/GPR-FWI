...
def main(optimizer=None, opt_params=None, epochs=2001):
    if rank == 0:
        if os.path.exists('results_rectangle'):
            shutil.rmtree('results_rectangle')
        os.makedirs('results_rectangle', exist_ok=True)
    ...
    # 真实模型参数（矩形异常体，与 gpr_train_mpi.py 对齐）
    epsilon_true = np.ones((xl, zl)) * 3
    epsilon_true[20:40, 25:55] = 1.5
    epsilon_true[85:, :] = 6
    sigma_true = np.zeros((xl, zl)) 
    ...
    if rank == 0:
        ...
        plt.colorbar()
        plt.savefig('results_rectangle/epsilon_true.png')
        plt.close()


    # 初始模型参数（与 gpr_train_mpi.py 对齐）
    epsilon0 = np.ones((xl, zl)) * 3
    epsilon0[55:, :] = 6
    sigma0 = np.zeros((xl, zl)) 
    ...
    if rank == 0:
        ...
        plt.colorbar()
        plt.savefig('results_rectangle/epsilon_initial.png')
        plt.close()
    ...
    if rank == 0:
        ...
        plt.figure(); plt.imshow(d_obs.reshape(len(source_list)*len(receiver_list), steps).T, aspect='auto', cmap='seismic'); plt.colorbar(); plt.title('Observed data'); plt.savefig('results_rectangle/d_obs.png'); plt.close()
    else:
        d_obs = None
    ...
    def run_once(name, params):
        ...
        if name == 'Muon' and isinstance(params, dict) and params.get('lr') == 0.01:
            exp_name = '0.01Muon'
        base = os.path.join('results_rectangle', exp_name)
        if rank == 0:
    ...
    if rank == 0:
        print('FWI完成！结果保存在results_rectangle文件夹中。')
