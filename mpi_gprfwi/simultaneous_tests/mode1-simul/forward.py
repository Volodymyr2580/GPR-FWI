...
from Time_loop import time_loop, time_loop_simul
...
def forward_model_simul(epsilon, sigma, source_list, receiver_list, dt, dx, dz, npml, freq, steps=1000, save_wavefield=False):
    xl, zl = epsilon.shape
    n_src = len(source_list)
    n_rec = len(receiver_list)
    #对simultaneous激发模式需要调整计算区域大小，添加炮集和高斯振幅衰减
    expand_size = 100
    epsilon_extended = np.ones((xl + 2*npml, zl + 2*npml))
    sigma_extended = np.ones((xl + 2*npml, zl + 2*npml)) * 0
    mu_extended = np.ones((xl + 2*npml, zl + 2*npml))
    ...
    t = np.arange(0, steps * dt, dt)
    wavelet = ricker(t, freq)
    data = np.zeros((n_rec, steps))
    source_list_ext = [(p[0]+npml, p[1]+npml) for p in source_list]
    receiver_list_ext = [(p[0]+npml, p[1]+npml) for p in receiver_list]
    shot_wavefield = []
    time_loop_gen = time_loop_simul(
        xl,
        zl,
        dx,
        dz,
    ...
    for step_idx, (ey_field, rec_vec) in enumerate(time_loop_gen):
        data[:, step_idx] = rec_vec
        if save_wavefield:
            shot_wavefield.append(np.array(ey_field).copy())
    if save_wavefield:
        return data, shot_wavefield
    else:
        return data