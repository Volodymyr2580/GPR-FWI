...
def time_loop(xl,zl,dx,dz,dt,sigma,epsilon,mu,CPML_Params,f,k_max,source_site,ref_pos):
    ...
    for tt in range(k_max):
        ...
        # pyplot.pause(0.01)
        # 4. 返回当前时间步的结果，并且是yield形式，返回Ey和ref_pos指定接收器位置的电场值
        yield Ey,Ey[ref_pos[0],ref_pos[1]]

def time_loop_simul(xl,zl,dx,dz,dt,sigma,epsilon,mu,CPML_Params,f,k_max,source_sites,ref_positions):
    ep0 = 8.841941282883074e-12
    mu0 = 1.2566370614359173e-06
    epsilon = epsilon.copy()*ep0
    mu = mu.copy()*mu0
    ...
    right_width = 50
    k = 5.0
    denom = (1.0 - np.exp(-k * left_width))
    source_decay = []
    for sx, sz in source_sites:
        z0 = sz - npml
        if z0 < left_width:
            d = z0
            factor = (1.0 - np.exp(-k * d)) / denom
        elif z0 > (zl - right_width):
            d = (zl - z0)
            factor = (1.0 - np.exp(-k * d)) / denom
        else:
            factor = 1.0
        source_decay.append(factor)
    for tt in range(k_max):
        Hz,Hx=update_H(xl,zl,dx,dz,dt,sigma,epsilon,mu,npml,a_x_half,a_z_half,b_x_half,b_z_half,k_x_half,k_z_half,Hz,Hx,Ey,memory_dEy_dx,memory_dEy_dz)
        Ey=update_E(xl,zl,dx,dz,dt,ca,cb,npml,a_x,a_z,b_x,b_z,k_x,k_z,Hz,Hx,Ey,memory_dHz_dx,memory_dHx_dz)
        for idx, (sx,sz) in enumerate(source_sites):
            Ey[sx][sz]+= -cb[sx][sz]*source_decay[idx]*f[tt]*dt/dx/dz
        rec = np.array([Ey[rx,rz] for rx,rz in ref_positions])
        yield Ey, rec
...
def reverse_time_loop(xl,zl,dx,dz,dt,sigma,epsilon,mu,CPML_Params,k_max,ref_pos,rhs_data):
    ...
    for tt in range(k_max):
        #由于一阶差分，反传时source term上会多一个负号，之前是我想简单了。
...
def reverse_time_loop_simul(xl,zl,dx,dz,dt,sigma,epsilon,mu,CPML_Params,k_max,ref_positions,rhs_data):
    ep0 = 8.841941282883074e-12
    mu0 = 1.2566370614359173e-06
    epsilon = epsilon.copy()*ep0
    mu = mu.copy()*mu0
    ...
    ca = CPML_Params.ca_r
    cb = CPML_Params.cb
    nrec = len(ref_positions)
    # 预计算接收点边界指数衰减系数（用于伴随源注入）
    left_width = 50
    right_width = 50
    k = 5.0
    denom = (1.0 - np.exp(-k * left_width))
    rec_decay = []
    for rx, rz in ref_positions:
        z0 = rz - npml
        if z0 < left_width:
            d = z0
            factor = (1.0 - np.exp(-k * d)) / denom
            ...
            factor = 1.0
        rec_decay.append(factor)
    for tt in range(k_max):
        for i in range(nrec):
            rx,rz = ref_positions[i]
            Ey[rx,rz]-=rec_decay[i]*rhs_data[i, k_max-tt-1]
        Hz,Hx=update_H(xl,zl,dx,dz,dt,sigma,epsilon,mu,npml,a_x_half,a_z_half,b_x_half,b_z_half,k_x_half,k_z_half,Hz,Hx,Ey,memory_dEy_dx,memory_dEy_dz)
        Ey=update_E(xl,zl,dx,dz,dt,ca,cb,npml,a_x,a_z,b_x,b_z,k_x,k_z,Hz,Hx,Ey,memory_dHz_dx,memory_dHx_dz)
        yield Ey
