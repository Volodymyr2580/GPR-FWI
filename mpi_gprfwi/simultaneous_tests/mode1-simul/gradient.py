...
import sys
import os
from Time_loop import reverse_time_loop, reverse_time_loop_simul
...
def compute_gradient_simul(epsilon, sigma, residual, source_list, receiver_list, dt, dx, dz, npml, freq, steps, sigma_required_gradient=False, wavefield_data=None):
    xl, zl = epsilon.shape
    ep0 = 8.841941282883074e-12
    epsilon_extended = np.ones((xl + 2*npml, zl + 2*npml))
    sigma_extended = np.ones((xl + 2*npml, zl + 2*npml)) * 0
    ...
    sigma_extended[:, -npml:] = sigma_extended[:, -npml-1].reshape(-1, 1)
    cpml_params = Add_CPML(xl, zl, sigma_extended.copy(), epsilon_extended.copy(), mu_extended.copy(), dx, dz, dt)
    receiver_list_extended = [(p[0]+npml, p[1]+npml) for p in receiver_list]
    grad_eps = np.zeros_like(epsilon)
    grad_sig = np.zeros_like(sigma) if sigma_required_gradient else None
    reverse_loop_gen = reverse_time_loop_simul(
        xl,
        zl,
        dx,
        dz,
        ...
        cpml_params,
        steps,
        receiver_list_extended,
        residual,
    )
    if wavefield_data is not None and len(wavefield_data) > 0:
        forward_wavefield = np.array(wavefield_data)
        forward_diff = (forward_wavefield[2:] - forward_wavefield[:-2]) / (2 * dt)
        adjoint_list = [np.array(adj) for adj in reverse_loop_gen][::-1]
        for k in range(1, steps-1):
            adjoint_field = adjoint_list[k]
