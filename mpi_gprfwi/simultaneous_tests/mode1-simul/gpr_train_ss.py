...
def main():
    ...
    result_dir = f'result_gpr_{offset}_lr{learning_rate:.0e}_alpha{alpha_tikhonov_eps:.3g}/'
    ...
    epsilon_true[40:60, 490:510] = 1.5
    epsilon_true[85:, :] = 6
    ...
    # ========== 生成观测数据 ==========
    print("生成观测数据 (simul 模式)...")
    d_obs = forward_model_simul(
    ...
    plt.imshow(d_plot.detach().cpu().numpy().T, aspect='auto', cmap='seismic', vmin=-50, vmax=50)
    plt.title('Observed Data (d_obs extended)')
    plt.colorbar(); plt.tight_layout()
    plt.savefig(f"{path}d_obs_extended.png"); plt.close()

    plt.figure(figsize=(12, 4))
    plt.imshow(d_plot.detach().cpu().numpy()[200:300, :].T, aspect='auto', cmap='seismic', vmin=-10, vmax=10)
    plt.title('Observed Data (d_obs middle)')
    plt.colorbar(); plt.tight_layout()
    plt.savefig(f"{path}d_obs_middle.png"); plt.close()

    np.save(f'{path}d_obs_simul.npy', d_obs.detach().cpu().numpy())
    ...
    # 计算padding
    target_height, target_width = 100, 1000
